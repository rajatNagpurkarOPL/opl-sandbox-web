...
@Component({
  selector: 'app-check-bhoomi-status',
  templateUrl: './check-bhoomi-status.component.html',
  styleUrls: ['./check-bhoomi-status.component.scss']
})
export class CheckBhoomiStatusComponent implements OnInit {
  ...
  checkBhoomiStatusForm: FormGroup;
  ...
  constructor(private fb : FormBuilder, public sandboxService : SandboxService,private utils : Utils, private aesGcmEncryption: AesGcmEncryptionService) {
   ...
   ngOnInit(): void {
    ...
    this.checkBhoomiStatusForm = this.formBuilder.group({
      ...
      ownerNo: ['', Validators.required]
    });
  }

  onFormSubmit() {
    if (this.checkBhoomiStatusForm.valid) {
      this.checkBhoomiStatusDetail(this.checkBhoomiStatusForm.value);
    } else {
...
checkBhoomiStatusDetail(requestedData : any){
