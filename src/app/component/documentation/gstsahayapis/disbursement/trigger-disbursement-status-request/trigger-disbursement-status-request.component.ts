...
@Component({
  selector: 'app-trigger-disbursement-status-request',
  templateUrl: './trigger-disbursement-status-request.component.html',
  styleUrls: ['./trigger-disbursement-status-request.component.scss']
})
export class TriggerDisbursementStatusRequestComponent implements OnInit, Websocket {
  ...
  topic: string = "/triggerDisbursementStatusResponse";
  ...
  saveData(){
    ...
    this.lenderService.triggerDisbursementStatusRequest(data).subscribe(res => {
  ...
  tabClick(tab) {
    if(tab.index==0){
      this.getApiRequestSchema('triggerDisbursementStatusRequest');
      this.getApiResponseSchema('triggerDisbursementStatusResponse');
    }else if(tab.index==1){
