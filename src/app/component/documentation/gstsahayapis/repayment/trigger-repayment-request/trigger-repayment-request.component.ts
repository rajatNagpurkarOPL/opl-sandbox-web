...
export class TriggerRepaymentRequestComponent implements OnInit, Websocket {
  ...
  topic: string = "/triggerRepaymentResponse";
  ...
  tabClick(tab) {
    if(tab.index==0){
      ...
      this.getApiRequestSchema('triggerRepaymentRequest');
      this.getApiResponseSchema('triggerRepaymentResponse');
    }else if(tab.index==1){
